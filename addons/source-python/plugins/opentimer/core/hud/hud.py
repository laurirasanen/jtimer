...
def draw_timer(player):
    ...
    if player.state.timer_mode == state.Timer_Mode.MAP:
        if player.state.map_state == state.Run_State.START:
            ...
            text = HintText(
                f"map end, time: {player.state.map[2] - player.state.map[1]}"
            )
        ...
        if player.state.course_state == state.Run_State.START:
            ...
            text = HintText(
                f"course end, time: {player.state.courses[0][2] - player.state.courses[0][1]}"
            )
        ...
        if player.state.bonus_state == state.Run_State.START:
            ...
            text = HintText(
                f"bonus end, time: {player.state.bonus[2] - player.state.bonus[1]}"
            )
